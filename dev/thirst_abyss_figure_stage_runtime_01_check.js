...
// 〔승계 — F8B-2(docs/72)〕 F8B-2가 같은 case에 보스 몸 반응(srEventReaction)을 **추가**했다.
// 의미 승계: 기존 화면 레벨 표현(SFX/shake/vign)은 원문 그대로이고 순서도 앞이며, drain 발화 지점(CORE)과
// Stage Signal 문맥 분리도 불변 — 보스 anchor 미사용도 그대로(reaction은 anchor를 쓰지 않는다).
chk('g3 drain 결과 표현 무변경(FX(aoe)=화면 레벨 원문·보스 anchor 미사용)',
  inSrc("FX('aoe');break}") &&
  inSrc("case 'aoe':SFX.aoe();shake();vign();srEventReaction('aoe');") &&
  inSrc("o.intent=(boss==='shell_thirst')?'drain':'judge';break;") &&
  strip(fnBody('srEventReaction')).indexOf('resolveAnchor') < 0, '');
...
} catch (e) { chk('k1 스모크(예외)', false, e.message); }
{
  ...
  chk('k3 index.html 신 기준선(231,444 B · md5 34281b01…)',
    buf.length === 231444 &&
