...
chk('c6 shell_iron 게이트 + 3동료 스위칭',
  ...
  hasH('#stage.sb-fig-stage #sb-rog-fig') && hasH('#stage.sb-fig-stage #sb-mage-fig') &&
  hasH('#stage.sb-fig-stage #sb-sham-fig'), '');
...
chk('c17 index.html 현행 기준선(223,967 B · md5 1fa9132f…)',
  buf.length === 223967 &&
...
// 〔승계 — F6(docs/68 §15)〕 파티 배치가 보스 figure 무대 공통 문맥(.sb-fig-stage)으로 이동하고, 문맥 토글은 Profile 기반
// sbStageContext()가 소유. 보호 의미 불변: figure profile 없는 보스(심연)는 문맥 class 0 → 전역 stageAllies bottom:6 유지.
chk('c21 다른 보스 회귀 보호(figure 무대 스코프·전역 stageAllies bottom:6)',
...
// 25. div/section 균형 (222/222·8/8)
{
  const dO = cntH('<div'), dC = cntH('</div>'), sO = cntH('<section'), sC2 = cntH('</section>');
  chk('c25 div/section 균형(222/222·8/8)', dO === dC && dO === 222 && sO === 8, dO + '/' + dC + ' · ' + sO + '/' + sC2);
}
