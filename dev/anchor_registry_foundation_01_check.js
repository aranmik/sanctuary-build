...
// 〔승계 — F6(docs/68 §17)〕 owner 모델 정밀화: 보스 figure는 자기 전투 소유(bossId 문맥) / 동료는 전 보스의 파티원(context-free·
// 가시성은 무대 CSS가 소유하고 숨김 figure는 rect-zero로 이미 fail-closed). 따라서 iron-scoped 5 → 보스별 문맥 2(iron·morgas)로 정정.
// 의미 불변: 모든 등록 항목이 owner(문맥)·binding(figId)·라벨을 갖는다.
chk('b1 등록 항목에 owner/binding/라벨 존재(stageContext·figId·part·label)',
  (f3.match(/stageContext:'shell_iron'/g) || []).length === 1 &&
  (f3.match(/stageContext:'boss01'/g) || []).length === 1 &&
  (f3.match(/stageContext:null/g) || []).length >= 6 &&
  (f3.match(/figId:'sb-/g) || []).length === 6 && (f3.match(/label:'/g) || []).length >= 6, '');
...
// 〔승계 — F6(docs/68 §16)〕 모르가스 figure anchor(boss_morgas·무대 선 원점 1종) 등록으로 목록 확장 — 기존 항목 순서·의미 불변
...

{
  ...
  chk('e3 index.html 현행 기준선(223,967 B · md5 1fa9132f…)',
    buf.length === 223967 &&
