...
// #stage.sb-fig-stage #sa-* translate(x,y) 추출
function saT(id) {
  // 〔승계 — F6(docs/68 §15)〕 파티 배치 규칙이 보스별(.sb-boss-iron)에서 보스 figure 무대 공통(.sb-fig-stage)으로 이동 —
  // 선택자만 공통화되고 translate 상수는 byte 불변. 아래 배치 검증(4인 원호·좌우 분산·스팬)의 의미는 그대로.
  const re = new RegExp('#stage\\.sb-fig-stage #' + id + '\\{transform:translate\\((-?[0-9.]+)px,(-?[0-9.]+)px\\)');
...
//    ★스코프 승계(F6·docs/68 §15): shell_iron 전용 → 보스 figure 무대 공통(.sb-fig-stage). gap 26·bottom 20 상수는 byte 불변.
chk('c6 stageAllies gap 상향 조정(26, 이전 12 대비 증가)',
  /#stage\.sb-fig-stage #stageAllies\{bottom:20px;gap:26px\}/.test(html), '');
...
chk('c20 index.html 현행 기준선(227,650 B · md5 5d645ffc…)',
  buf.length === 227650 &&
...
// 〔승계 — F6(docs/68 §15)〕 배치 스코프가 보스 figure 무대 공통(.sb-fig-stage)·문맥 토글은 Profile 기반 sbStageContext().
// 보호 의미 불변: figure profile 없는 보스(심연)는 문맥 class를 받지 못해 전역 stageAllies bottom:6을 그대로 쓴다.
chk('c24 다른 보스 회귀 보호(figure 무대 스코프·전역 stageAllies bottom:6)',
  hasH('#stageAllies{position:absolute;bottom:6px') &&
  hasH('sC(st,p.contextClass,p===act)') &&
...
// 27. div/section 균형 (222/222·8/8)
{
  const dO = cntH('<div'), dC = cntH('</div>'), sO = cntH('<section');
  chk('c27 div/section 균형(222/222·8/8)', dO === dC && dO === 222 && sO === 8, dO + '/' + dC + ' · sec ' + sO);
}
