...
// 〔승계 — F2(docs/63)〕 감사 당시 if/else 코드가 등가 registry 규칙으로 치환됨 — 조건 의미(생존∧보호막→guard / 생존∧강타 예고→brace·순서=우선순위) 불변을 신 형태로 검증
...
// 〔승계 — F2(docs/63)〕 smashing?'windup':'' 코드가 등가 registry 규칙으로 치환됨 — smash cast 전용 조건 의미 불변
chk('c10 파쇄자 windup 조사 + 실코드 일치',
  inDoc('windup pose 입력', 'sb-ic-hammer') &&
  inSrc("{pose:'windup',when:{bossActionKind:'smash'}}") && inSrc('sb-ic-hammer'), '');
...
// 21. ★감사 핵심 발견 기록 — fxAnchors bossAvatar 퇴화 (display:none 원인은 존치)
// 〔승계 — F3 closeout(docs/64)〕 감사 당시 fxAnchors의 c('bossAvatar') 유령 좌표 → F3에서 resolveAnchor('boss','avatar')로
// 이관되어 display:none 시 rect-zero→null로 봉인됨. 원인 CSS(display:none)와 감사 발견 문서는 그대로, 유령 좌표는 이제 발생 0.
// 〔승계 — F6(docs/68 §15·§17)〕 아바타 숨김 규칙이 보스별(.sb-boss-iron) → 보스 figure 무대 공통(.sb-fig-stage)으로 이동.
// 봉인 의미 불변: figure 무대에선 이모지 아바타가 숨겨지고 그 anchor는 rect-zero→null(유령 좌표 0). legacy 폴백 경로도 그대로 존치.
chk('c21 핵심 발견(fxAnchors 유령 좌표) + 실코드 근거(F3 봉인 승계)',
  inDoc('유령 좌표', 'Rework 02의 1순위 수정 지점') &&
  inSrc("resolveAnchor('boss','avatar')") && inSrc('#stage.sb-fig-stage #bossAvatar{display:none}'), '');
...
// 23. index.html md5 기준선 유지 (무변경)
{
  ...
  chk('c23 index.html 무변경(231,444 B · md5 34281b01…)',
    buf.length === 231444 &&
