...
- Shaman Sustain Support Cue 01 / F4A ([66](./66_SHAMAN_SUSTAIN_SUPPORT_CUE_01.md) · EP26 · **주술사 sustain 가독성 보강 — 첫 Stage-owned one-shot cue** — valorActive false→true 경계 1회: 지원 파동(`sham`/`body` anchor)+파티 머리 위 푱! ping(신규 `head` anchor 4종·생존 Actor만·hidden/0-rect=null 생략·(0,0) 봉인)→은은한 hold pulse·lifecycle=CSS animation+animationend+generation+setTimeout Stage-only 안전망·scHardReset(boss switch/newGame/battle end stale 0)·gameplay/valor/S.ev/drain/공통 resolver/전사·마법사·도적 무접촉·cue ON/OFF 등가·CORE byte-identical·스모크 불변 · 재-baseline 185,737/2,545/`8d72d049…` · `shaman_sustain_support_cue_01_check` 33/33 · F4 FINAL PASS closeout 조건·F5 입력 계약=docs/66 §23 · **F4/F4A FINAL PASS·커밋 `08728af`**)
- Boss Figure Shell / Face Template 01 / F5 ([67](./67_BOSS_FIGURE_SHELL_FACE_TEMPLATE_01.md) · EP26 · **골격은 공통, 연기는 고유 — 보스 확장 기반** — BOSS_STAGE_PROFILE[bossId](데이터=보스별)+BOSS_FIGURE_SHELL[shellId](sb_unit_v1=현행 3층 골격 서술)+generic 9함수(bossId·보스 이름 literal 분기 0·미등록 boss/shell=null·프로토타입 차단·등록 순서 의존 0)·**binder 방식**(DOM 생성 0→duplicate root 구조적 0)·**Pose/Face 분리**(data-pose 몸 / data-face 얼굴·조건 해석은 F2 sgPoseCond 재사용)·실 truth만 연결(idle→neutral·smash→tell·hurt/groggy/defeated는 candidateFaces 보존·가짜 timer 0)·단일 파츠 얼굴 허용·optional slot(sideL/sideR) null·신규 anchor 0·owner cleanup(문맥 밖 data-face 제거) · **등가 2겹**=971틱 face tell⟺pose windup mismatch 0 + pane 전수 diff 차이 2건(data-face 속성뿐·픽셀 변화 0) · CORE byte-identical·재-baseline 194,919/2,677/`33d20ae3…` · `boss_figure_shell_face_template_01_check` 48/48 · 승계 2건(F4A render 배선 형태 핀) · **FINAL PASS/RELEASED·커밋 `90e029a`** · F6 입력 계약=§26 · F7 입력 계약=§27)
- Morgas Figure & Stage Runtime 01 / F6 ([68](./68_MORGAS_FIGURE_STAGE_RUNTIME_01.md) · EP26 · **두 번째 보스 production figure — 모르가스가 질문하고, 도적이 대답한다** — F5 계약 충분성 입증(Profile 데이터+고유 DOM/CSS+signal mapping만으로 등록·**F5 generic 9함수/F2·F3 resolver 무수정**·literal 분기 0)·★감사 3발견(도적은 이미 준비돼 있었고 막은 건 규칙이 아니라 문맥 / 파티 배치가 보스에 묶임 / 행동선 원점이 이모지)·**`.sb-fig-stage` 공통 무대 문맥 ↔ `.sb-boss-*` 고유 figure 표시 2축 분리**+Profile 기반 `sbStageContext()`+동료 context-free(가시성=CSS·숨김=rect-zero fail-close)+`resolveActiveBossAnchor('avatar')||legacy`(파쇄자는 미선언→null→계보 무변경)·**모르가스=꼬는 자**(승인 [39]D-1/[41]H-1·`sb_unit_v1` 재사용·fig 120×198·7파츠·핏빛/보라)·**pose weave(몸) / face tell(얼굴) 분리·다중 파츠 얼굴 첫 사례**·**도적 interrupt 실가시**(F4 규칙 무수정 자동 발동)·차단선 도적→모르가스 몸통 · **파쇄자 회귀 전수 diff 0**·파티 배치 iron=morgas 동일·심연 무접촉·스모크 3종 불변·CORE byte-identical · 재-baseline 205,777/2,753/`dd4e0405…` · `morgas_figure_stage_runtime_01_check` 51/51·전체 38종 PASS · 승계 12건(형태 핀·의미 불변) · **FINAL PASS·커밋 `e785212`** · F7 입력 계약=§31 · F8 경계=§32)
- Thirst Abyss Figure & Stage Runtime 01 / F7 ([69](./69_THIRST_ABYSS_FIGURE_STAGE_RUNTIME_01.md) · EP26 · **비인간형 보스 — 3보스 전원 production figure 완료** — `심연은 공격 전에 위협적인 게 아니라 존재하는 동안 계속 말리고 있다`·★감사(심연은 cast를 아예 안 함→bossAction 영구 null / A.boss 소비자 2곳 모두 cast 필요→심연은 미사용→anchors:[] / 마법사 상시 channel은 F4 truth) · **F5 경계 4종 실증**(faceParts 자체 없음=얼굴 없는 보스 / weapon·ornament null / **sideL·sideR 첫 소비자**·크기·주기·위치 전부 비대칭 / anchors:[] 미사용 선등록 ⛔) · **shell `sb_unit_v1` 3번째 재사용**(새 shell 불요) · **pose/face 0**(cast truth 없음→poseRules[]·faceRules[]·default only)·**개성은 ambient CSS**(core 안으로 호흡·오라 말려듦·조각 어긋난 주기·느린 부유)·**가짜 tell 0**(실 전투 1236틱 bossAction 0·timer 0·안으로 수렴만) · ★`enraged`(t=56)는 실 truth이나 pose 연결엔 sgPoseCond 조건 키 추가(F2 generic 수정) 필요 → **발주 §6대로 HOLD·보고만**(3보스 공통 공백·F8 계약) · **파쇄자·모르가스 회귀 전수 diff 0(둘 다)**·파티 좌표 3보스 동일·겹침 0·스모크 불변·CORE byte-identical · 재-baseline 213,295/2,800/`afe3de3a…` · `thirst_abyss_figure_stage_runtime_01_check` 54/54·전체 39종 PASS · 승계 6건 · **FINAL PASS/RELEASED·커밋 `f8786b7`** · F8 입력 계약=§34 · F9 closeout 계약=§35)
- Boss Enrage & Hit Reaction Grammar 01 / F8A ([70](./70_BOSS_ENRAGE_HIT_REACTION_GRAMMAR_01.md) · EP26 · **전투극 1막 — 숫자가 피해를 말하고, 몸이 충격을 증명한다** — 채널 2종 신설 `data-state`(지속 enraged)+`data-reaction`(one-shot hit)·4채널 공존(실측 windup+tell+enraged+hit 동시)·★감사 3발견(enrage=단일 truth·snapshot 기존재 / hit=HP 단일 경로+기존 이벤트 `FX('allyAtk')` §7 우선순위 1 / **.sb-react transform은 pose 소유→속성 채널 분리**: pose=transform / reaction=**개별 속성 translate·rotate·scale** 네이티브 합성=keyframes transform: 직서 0 봉인) · generic rail(sgPoseCond+`enraged` 키 1개[발주 승인]·resolveActorState/presentActorState/srBossHit/sbBossState·bossId 분기 0·profile 데이터 stateRules/reactions{anim,ms}) · F4A lifecycle(gen+animationend 1회 바인딩+Stage-only timeout 1개)+**게임시간 0.12s coalescing**(frozen-tab 안전·실 전투 hits 14/coalesced 2) · **보스별 연기**: 파쇄자 버틴다(-3.5px+hammer 반박자 lag) / 모르가스 꺾인다(횡 snap+가면 recoil) / 심연 삼킨다(수축+하강·outward 0·가짜 tell 여전히 0)·enrage=filter/주기만(형태 무변) · decal pop/smash tell/cue/파티 회귀 0·스모크 불변·CORE byte-identical·div 추가 0 · 재-baseline 223,967/2,934/`1fa9132f…` · `boss_enrage_hit_reaction_grammar_01_check` 54/54·전체 40종 PASS(당시 실제 열거) · 승계 1건(F7 j5 키 계보) · **FINAL PASS/RELEASED·커밋 `52245fd`** · F8B 입력 계약=§30 · F9 계약=§31)
- Morgas Interrupt Result Reaction 01 / F8B-1 ([71](./71_MORGAS_INTERRUPT_RESULT_REACTION_01.md) · EP26 · **전투극 2막 — 끊겼다는 사실을, 보스의 몸이 인정한다** — F6 승인 장면(judge→도적 interrupt→차단선→데칼 팟)에 마지막 한 박자 · ★truth 감사(성공 확정=`tryInterrupt` 실패 3사유 조기 return 뒤 `FX('intOk')` 1곳 / **데칼 소멸과 동일 truth 증명**=성공의 `S.boss.cast=null`을 같은 프레임 renderFx가 읽음 → 자동 동기·임의 delay 0) · **rail 최소 일반화**(`srTrigger(key)` generic 승격·coalescing/priority/anim/ms 전부 profile 데이터·어휘별 맵+활성 추적·animationend 등록 어휘 순회) · **priority 계약**(interrupted 30 > hit 10·coalesce 0=1성공 1반응·hit active 중 교체/interrupted 중 hit는 시각만 양보 suppressed·**key literal 비교 0·bossId 분기 0**) · 연기(축 붕괴+부유 하강 / 실 풀렸다 회복 / 가면 꺾임·개별 속성만·일반 hit .16s와 육안 구분) · **실 전투 t=15.80 실측**(3애니 동시+같은 프레임 데칼 opacity 0+차단선 off) · false-positive 0(judge 중 발화 0·intFail 소비 0·CSS/데칼/pose 추측 0) · 4채널 공존·pose matrix 소수점 보존·anchor 복원 · **F6 장면 전부 원문 무변경**·스모크 불변·CORE byte-identical·div 추가 0 · 재-baseline 227,650/2,972/`5d645ffc…` · `morgas_interrupt_result_reaction_01_check` 47/47·전체 41종 PASS(실제 파일 열거) · 승계 7건(F8A rail 이동·계약 불변) · **나라님 Phone Human Gate 대기·commit/push 0** · F8B-2 입력=§31 · F9 계약=§32)
- Thirst Abyss Drain Result Reaction 01 / F8B-2 ([72](./72_THIRST_ABYSS_DRAIN_RESULT_REACTION_01.md) · EP26 · **전투극 3막 — 빼앗은 것을 심연이 삼킨다** — F7 심연 ambient 위에 실제 drain 결과 순간의 signature reaction·**예고 영구 0**(cast/tell/windup 경로 부재·bossAction null 유지) · ★drain truth(`case 'drain'` t=5·11·19…71 **11회**·마나 −62/광폭 95 → 파티+사제 −42/광폭 64 → **`FX('aoe')` 1회는 결과 적용 완료 뒤**) · **★CORE 딜레마와 정본 해법**(`FX('aoe')`를 심연 drain[1205]과 모르가스 judge 착지[1243]가 공유·**둘 다 CORE 안**이라 payload 보강 불가·Stage Signal intent는 bossId 파생이라 저촉 → **presentation metadata를 boss profile이 소유**: `reactionEvents:{aoe:'drain'}` + generic `srEventReaction(evKey)` 조회만 → **judge aoe 무해 no-op 실측 0**·bossId 분기 0·SCRIPT 시간 0·CSS/DOM 읽기 0) · timing(기존 screen FX 원문 뒤 같은 frame·delay 0) · priority(drain 30 > hit 10·coalesce 0) · **★ambient 무손상 전략**(base 애니 있는 파츠엔 animation 미부여 → keyframes는 `.sb-react`/**`.sb-ab-core::before`**/`.sb-shadow` 3곳, aura·pool·shardL·shardR은 **transition 개별 속성** → base 애니 이름 before/during/after 전 구간 동일·복귀 jump 0) · 연기(중심 공허 1.62배 주연 / 조각 부호 반대 +7px·−6px·속도 .15s·.23s 비대칭 / root recoil 얕게 .972 vs hit .958) · **실 전투 t=5.05 실측**(마나 −62·HP −168과 같은 프레임·coreBaseAnim 보존·bossAction null·pose/face 빈 값) · **광폭 공존 실측**(core 3.1s + reaction 0.34s·해제 후 4.4s 복원) · screen FX/로그/aoes/drain 수치·시점 **전부 원문**·div 추가 0·스모크 불변·CORE byte-identical · 재-baseline 231,444/3,011/`34281b01…` · `thirst_abyss_drain_result_reaction_01_check` 51/51·전체 42종 PASS(`ls dev/*_check.js` 실제 열거) · 승계 1건(F7 g3) · **나라님 Phone Human Gate 대기·commit/push 0** · F8B-3 입력=§33 · F9 계약=§34)
