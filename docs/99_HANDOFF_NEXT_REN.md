...
**EP24 · Iron Crusher Smash Line Rework 02**: **첫 실전 무대 언어 구현**([55](./55_IRON_CRUSHER_SMASH_LINE_REWORK_02.md)·p24·기준 HEAD `a59d85e` clean·index 149,440→**155,043/2,185/`154ee46e`**·CORE 6cad2ec2 byte-identical·div 214 불변·**DOM 추가 0**). ①**유령 좌표 제거**: shell_iron에선 fxAnchors/캐시 A를 아예 안 쓰고 `sbSmashFx`가 **매 render 파츠 rect 직독**(sbPt) — source=`.sb-ic-hammer` 자루 상단 12%(머리=pseudo라 근사)·target=`bc.tg`가 전사면 (blk‖cast관측시 보호막)?`.sb-w-shield`:`.sb-w-body`, 비전사면 SB_BODY 맵의 실제 body element·`on=!!(src&&dst)`로 모서리/(0,0) fallback 0. ②게이트: `CUR_BOSS==='shell_iron'` 선행(모르가스 smash kind 공유)·비-iron은 기존 `fxLn(...'fxBoss')` 원문 경로(byte 의미 보존). ③관계선: 기존 fxBossLine 재사용·`fxSmash` 철·용광로(#7d5a34→commit #a97b3f·ember shadow)·pr=cast 진행률로 intent/commit(새 시간 0). ④**resolve 3근거 교차**: `sm.S===S && t>=sm.end-0.05 && sm.aliveL`(전환/재시작/미스 배제·smash는 중도 소거 경로 없음). ⑤**접촉 4분기**: `sm.blk`(각본)×`S.st.abs-sm.absL`(같은 tick delta) → sbhit-blk(철 brightness)/sbhit-abs(청광)/sbhit-blkabs(한 사건: 철 주연+청 여운)/sbhit-body(fig jolt+몸 flash)·**resolve 후 현재 shield 참조 0**(체크 c12가 구조 검증). ⑥SB_SM(render층 스냅샷·S 저장 0)+SB_DIRTY(보스 전환 정리)+animationend cleanup(새 setTimeout 0)·방패 keyframe은 **filter만**(guard transform 충돌 회피). **실런타임 검증**(★frozen-tab이라 실엔진 `G.step` 구동=스모크 방식·가짜 상태 0·보호막은 실제 버튼 클릭): 4분기 전부+비전사(도적 몸통 선+접촉)+모르가스 fxBoss 유지+심연 잔재 0+보스 전환 cleanup+overflow 0+콘솔 0·**보호막 전소→null인데 delta +320으로 흡수 포착**(오판 방지 실증)·캡처 2종(windup 몸통선/blk 방패선·헤드리스). 재-baseline: baseline+라이브 핀 19파일(bb7fc147→154ee46e). ★함정 3: (a)docs/53·54 체크의 "p24.mjs 부재" 단언=미래 파일명 과잉 고정 → 근거 주석과 함께 정정(보고됨) (b)신규 블록 슬라이스가 동명 CSS 주석에 걸려 기존 fxBeam setTimeout 오탐→JS 마커로 슬라이스 (c)자기 주석 "새 setTimeout 없음"이 setTimeout 문자열 매치→호출형 `setTimeout(`으로 좁힘. **27종 전체 PASS·스모크 3보스 동일**. 알려진 한계: 모르가스 A 캐시 스테일(선재·[54 §19])·blk+비전사 단순화·망치 머리 근사. 다음: 나라 실기→Companion Pose Runtime 01/색 채택안 이식/압력 데칼/Morgas Connection.

**EP24 · Battle Stage Vertical & Command Rebalance 01**: **전장 세로 환원+명령 피드백 재구성**([56](./56_BATTLE_STAGE_VERTICAL_COMMAND_REBALANCE_01.md)·기준 HEAD `8d4449b` clean·index 155,043→**156,106/2,193/`ad2a4a4d`**·CORE 6cad2ec2 불변·div 214·**DOM 추가 0·p파일 0**=직접 편집 8곳을 docs/56 §25에 전부 기록). **레이아웃**: 상단 −13(hud pad 8/9→6/6·bossbar mt7→5·castwrap mt5→3[slim=사제바 무영향]·warnBar py6→4)+하단 −9(#gcdBar display:none·DOM/render 무접촉=sT/sW null가드 부재로 CSS 회수 선택)→**무대 338→360(+22·flex:1 자동 환원)**·스킬 하단 829→838(safe-area shim 6 유지)·보스 fig 위로13/전사 아래로15(**세로 대치 +28**·top-anchor 자동+allies bottom 26→20 shell_iron 스코프)·버튼 120×60 불변·375/390/430 3뷰포트 overflow 0. **§7 바 감사**: #gcdBar=순수 GCD(A·p.gcd)·priCastWrap=실시전(B·HUD 내=허용 위치·유지)·개별cd(C)=기존 버튼 veil+카운트다운·D없음/E아님. **피드백**: gLock .78→.66+`.skill{transition:opacity .18s}`(자연 복귀)·**casting class+`--cp` 진행선**(render 4줄·`100*(1-(p.cast.end-t)/p.cast.dur)` 실진행·하단 3px 금빛=priCast 언어·시전 버튼만 밝음)·noMana 빨강 비용 유지=4상태 구분. **실검증**(pane 실엔진 G.step): gLock 사이클 잔재0·casting 53% 정확·gcd숨김·Rework 02 강타선 신좌표 정확 추적(244,113=망치)·모르가스/심연 회귀0·콘솔0. ★캡처 A/E/F 확보·**B/C 정지컷은 headless 가상시간이 1.0~1.6s 상태를 지나쳐 미확보**(pane 수치 검증 대체·투명 보고). 재-baseline: baseline+live 핀 21파일(154ee46e→ad2a4a4d·155043→156106). ★**구계약 승계 1건(보고)**: party_layout c6의 `bottom:26px` 고정 단언 → 본 카드 승인값 20px로 상수 승계(gap 26 상향 취지 유지·근거 주석). 신규 battle_stage_vertical_command_rebalance_01_check 27/27·**전체 28종 PASS·스모크 동일**. 다음: 나라 폰 human gate(§18 10항)→유키 검수. 후속 여지: 카드/사제HUD 2차 압축(+10~20px)·gcdBar DOM 완전 제거(+sT/sW 가드).
